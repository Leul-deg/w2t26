...
(
  ...
  docker compose exec -T backend bash -lc "
    export DATABASE_URL='postgresql://lms_user:changeme@postgres:5432/lms?sslmode=disable'
    ...
    PGPASSWORD=changeme psql -h postgres -U lms_user -d postgres -c \"DROP DATABASE IF EXISTS lms_test WITH (FORCE);\"
    PGPASSWORD=changeme psql -h postgres -U lms_user -d postgres -c \"CREATE DATABASE lms_test OWNER lms_user;\"
    /usr/local/go/bin/go run ./cmd/migrate up >/dev/null
    DATABASE_URL=\"\$DATABASE_TEST_URL\" /usr/local/go/bin/go run ./cmd/migrate up >/dev/null
  "
)
